...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            var pendingDownloadTrack by remember { mutableStateOf<OnlineTrack?>(null) }
            ...
            NexoraTheme(
                ...
                Scaffold(
                    ...
                    AppContent(
                        ...
                        onRequestDownload = { track ->
                            when (state.preferences.downloadStorageMode) {
                                DownloadStorageMode.ASK_FIRST_TIME -> pendingDownloadTrack = track
                                else -> viewModel.downloadOnlineTrack(track, state.preferences.downloadStorageMode)
                            }
                        }
                    )
                }
                ...
                pendingDownloadTrack?.let { track ->
                    AlertDialog(
                        ...
                        text = { Text("Choose where NexoraPlayer should store this download the first time.") },
                        confirmButton = {
                            TextButton(onClick = {
                                viewModel.setDownloadStorageMode(DownloadStorageMode.PUBLIC_DOWNLOADS)
                                viewModel.downloadOnlineTrack(track, DownloadStorageMode.PUBLIC_DOWNLOADS)
                                pendingDownloadTrack = null
                            }) { Text("Visible folder") }
                        },
                        dismissButton = {
                            TextButton(onClick = {
                                viewModel.setDownloadStorageMode(DownloadStorageMode.APP_PRIVATE)
                                viewModel.downloadOnlineTrack(track, DownloadStorageMode.APP_PRIVATE)
                                pendingDownloadTrack = null
                            }) { Text("Only in app") }
                        }
                    )
...
private fun AppContent(
    ...
    onRequestDownload: (OnlineTrack) -> Unit
) {
    if (state.search.isNotBlank()) {
        SearchResultsScreen(
            ...
            onDownloadOnline = onRequestDownload,
    ...
    HorizontalPager(
        ...
        when (destinations[page]) {
            ...
            AppDestination.FAVORITES -> FavoritesScreen(
                modifier = Modifier.fillMaxSize(),
                favorites = state.favorites.filter { it.mediaKind == MediaKind.AUDIO.name },
            ...
            AppDestination.SETTINGS -> SettingsScreen(
                ...
                downloadStorageMode = state.preferences.downloadStorageMode,
                currentLanguage = rememberAppLanguage(),
                onThemeChange = viewModel::setThemeMode,
                onDynamicColorChange = viewModel::setDynamicColor,
                onOnlineMusicSearchChange = viewModel::setOnlineMusicSearchEnabled,
                onDownloadStorageModeChange = viewModel::setDownloadStorageMode,
